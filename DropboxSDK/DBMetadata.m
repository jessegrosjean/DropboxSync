...
+ (NSDateFormatter*) dateFormatter {
    ...
    NSDateFormatter* dateFormatter = [dictionary objectForKey:@"DBMetadataDateFormatter"];
    if (dateFormatter == nil) {
        dateFormatter = [[NSDateFormatter alloc] init];
		dateFormatter.locale = [[[NSLocale alloc] initWithLocaleIdentifier:@"en_US_POSIX"] autorelease];
		dateFormatter.dateFormat = @"EEE, dd MMM yyyy HH:mm:ss Z";
        [dictionary setObject:dateFormatter forKey:@"DBMetadataDateFormatter"];
...
- (id)initWithDictionary:(NSDictionary*)dict {
    if ((self = [super init])) {
        ...
        if ([dict objectForKey:@"modified"]) {
            lastModifiedDate = [[[DBMetadata dateFormatter] dateFromString:[dict objectForKey:@"modified"]] retain];
        }

        path = [[dict objectForKey:@"path"] retain];
		mimeType = [[dict objectForKey:@"mime_type"] retain];
...
- (id)initWithCoder:(NSCoder*)coder {
    if ((self = [super init])) {
        ...
        mimeType = [[coder decodeObjectForKey:@"mimeType"] retain];
